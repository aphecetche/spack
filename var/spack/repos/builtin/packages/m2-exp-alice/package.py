...
class M2ExpAlice(CMakePackage):
    ...
    variant('aliroot',default=False,description='use aliroot-lite library')
    variant('aliphysics',default=False,description='use aliphysics-lite library',when='+aliroot')
    ...
    depends_on('root',when='+tracking')
    depends_on('root',when='+aliroot')
    depends_on('root',when='+aliphysics')
    depends_on('root',when='+converters')
    depends_on('fmt')
